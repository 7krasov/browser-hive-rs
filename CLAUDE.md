...
**The span is the single source of request context — downstream log calls do NOT re-pass `ray_id`/`url`.** `worker/src/service.rs`, `worker/src/browser_pool.rs`, and `common/src/wait_strategy.rs` emit plain `info!/warn!("…")` and inherit `span_ray_id`/`span_url`/… from the active span. Filter uniformly in Loki by the `span_*` fields (`| json | span_url=~"…"`). The `ray_id` in `ScrapePageResponse` is a proto response field (not a log field) and is unrelated. **`spawn_blocking`/`spawn` boundaries must re-establish the span themselves**, since the span does not cross them: the wait-strategy and diagnostics blocking closures (`scrape_page_internal`, `collect_browser_diagnostics`) capture `tracing::Span::current()` and call `let _g = span.enter();` inside the closure; the `AlwaysNewContextGuard` Drop spawns its `destroy_context` future `.instrument(info_span!("always_new_context_drop", ray_id = …))`. The `BrowserPool` lifecycle monitor (a standalone background `tokio::spawn`, not request-scoped) logs a literal `ray_id = "lifecycle-monitor"` field instead of a span. Any new blocking/spawned code that logs must follow one of these two patterns or its lines will have no context.
